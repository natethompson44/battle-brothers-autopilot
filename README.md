...
| **Company Starts** | Three custom starting origins built for autonomous play: *Wolfeo's Company* (elite blinking avatar plus a nine-man line with war dogs), *Wolfeo's Hunt* (the same avatar with a small hunting pack) and *The Sworn Five* (an assassin avatar with four friends whose builds lean on each other, all with extra action points). Vanilla only. | `mod_company_starts_<version>.zip` |
