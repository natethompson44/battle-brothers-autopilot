...
| **Kit Pilot** | A Kit up button on the character screen equips the company from the stash: weapon, shield, armor, helmet, quiver by role, veterans first, upgrades when the stash has something clearly better, with a panel listing every change. Can also run on its own. | `mod_kit_pilot_<version>.zip` |
